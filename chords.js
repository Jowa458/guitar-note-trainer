...
function playChordStrum(q){
  const notes=q.v.notes.slice().sort((a,b)=>a.string-b.string),startTime=state.audio.currentTime+.004;
  const duration=Math.min(3,DURATION_BEATS[chValue('Rhythm')]*60/+chValue('Bpm')*.9),volume=1/Math.sqrt(notes.length);
  notes.forEach(n=>playMidi(n.midi,chValue('Tone'),duration,volume,startTime+(n.string-notes[0].string)*.01));
}
function chordTick(){if(ch.sub%DURATION_SUBDIVISIONS[chValue('Rhythm')]===0&&ch.started){ch.index++;fillChords();void warmChords()}ch.last=ch.sub;ch.started=true;if(ch.sub%DURATION_SUBDIVISIONS[chValue('Rhythm')]===0&&chChecked('Sound')){stopActiveSounds();const q=ch.queue[ch.index];playChordStrum(q)}if(ch.sub%2===0&&chChecked('Click'))click(ch.sub===0);renderChords();ch.sub=(ch.sub+1)%(+chValue('Beats')*2)}
