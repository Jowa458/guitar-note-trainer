...
function musicalLocation(locations,scaleNotes,context){
  if(Math.random()<.1)return choose(locations);
  ...
  return weightedChoice(locations,note=>{
    ...
    if(lastMidi==null)weight*=degree===0?6:degree===4?3.8:degree===2?3:1;
    else{const delta=midi-lastMidi,distance=Math.abs(delta);weight*=distance===0?1.1:distance<=2?5.4:distance<=4?2.8:distance<=7?1.15:.22;if(Math.abs(lastDelta)>=4&&Math.sign(delta)===-Math.sign(lastDelta)&&distance<=2)weight*=3.2;if(Math.abs(lastDelta)<=2&&Math.sign(delta)===Math.sign(lastDelta)&&distance<=2)weight*=1.25}
    if(strong&&stable)weight*=2.4;if(ending)weight*=degree===0?5:degree===4?2.3:degree===2?1.8:.65;
    return weight*(.65+Math.random()*.7);
  });
}
