...
let audioEpoch=0;
let metroBus=null,noteBus=null;
...
function choosePositionIndex(selected,context,nextKey=context?.key){
  ...
  const current=context.positionIndex,currentOffset=((context.key.pitch-4+12)%12)+POSITIONS[current].offset,groups={same:[],adjacent:[],near:[],far:[]};
  const distanceFor=index=>Math.abs(((nextKey.pitch-4+12)%12)+POSITIONS[index].offset-currentOffset);
  const groupFor=index=>{const distance=distanceFor(index);return distance<=1?'same':distance<=3?'adjacent':distance<=5?'near':'far'};
  selected.forEach(index=>groups[groupFor(index)].push(index));
  ...
  return weightedChoice(selected,index=>{
    const group=groupFor(index),direction=Math.sign(((nextKey.pitch-4+12)%12)+POSITIONS[index].offset-currentOffset);
    let weight=mass[group]/groups[group].length;
    if(index===current)weight*=Math.pow(.5,Math.max(0,(context.positionRun||1)-1));
...
function updateSharedVolumes(){
  for(const [id,bus] of [['metroVolume',metroBus],['noteVolume',noteBus]]){
    const level=+$('#'+id).value/100;$('#'+id+'Value').textContent=Math.round(level*100)+'%';
    if(bus){bus.gain.cancelScheduledValues(state.audio.currentTime);bus.gain.setTargetAtTime(level,state.audio.currentTime,.01);}
  }
}
async function ensureAudio(){state.audio||=new(window.AudioContext||window.webkitAudioContext)();if(!metroBus){metroBus=state.audio.createGain();noteBus=state.audio.createGain();metroBus.connect(state.audio.destination);noteBus.connect(state.audio.destination);updateSharedVolumes()}if(state.audio.state==='suspended')await state.audio.resume()}
function trackAudioVoice(source,gain,startTime=state.audio.currentTime){const voice={source,gain,startTime};activeAudioVoices.add(voice);source.addEventListener('ended',()=>activeAudioVoices.delete(voice),{once:true})}
function stopActiveSounds(){audioEpoch++;if(!state.audio)return;const now=state.audio.currentTime;activeAudioVoices.forEach(({source,gain,startTime})=>{try{if(startTime>now){source.stop(now);return}if(gain.gain.cancelAndHoldAtTime)gain.gain.cancelAndHoldAtTime(now);else{gain.gain.cancelScheduledValues(now);gain.gain.setValueAtTime(Math.max(.001,gain.gain.value),now)}gain.gain.exponentialRampToValueAtTime(.001,now+.025);source.stop(now+.03)}catch(error){}});activeAudioVoices.clear()}
function click(accent){const now=state.audio.currentTime,oscillator=state.audio.createOscillator(),gain=state.audio.createGain();oscillator.type=accent?'triangle':'sine';oscillator.frequency.value=accent?1380:680;gain.gain.setValueAtTime(.001,now);gain.gain.exponentialRampToValueAtTime(accent?.2:.075,now+.003);gain.gain.exponentialRampToValueAtTime(.001,now+(accent?.035:.055));oscillator.connect(gain).connect(metroBus);trackAudioVoice(oscillator,gain,now);oscillator.start(now);oscillator.stop(now+.065)}
...
function playFallbackMidi(midi,duration,volume=1,startTime=state.audio.currentTime){const frequency=440*Math.pow(2,(midi-69)/12),now=Math.max(state.audio.currentTime,startTime),oscillator=state.audio.createOscillator(),gain=state.audio.createGain();oscillator.type='triangle';oscillator.frequency.value=frequency;gain.gain.setValueAtTime(.001,now);gain.gain.exponentialRampToValueAtTime(.13*volume,now+.008);gain.gain.exponentialRampToValueAtTime(.001,now+duration);oscillator.connect(gain).connect(noteBus);trackAudioVoice(oscillator,gain,now);oscillator.start(now);oscillator.stop(now+duration+.03)}
function playSample(buffer,rate,instrument,duration,volume=1,startTime=state.audio.currentTime){const now=Math.max(state.audio.currentTime,startTime),source=state.audio.createBufferSource(),gain=state.audio.createGain(),release=Math.max(.3,duration),level=(instrument==='eguitar'?.42:.52)*volume;source.buffer=buffer;source.playbackRate.value=rate;gain.gain.setValueAtTime(.001,now);gain.gain.exponentialRampToValueAtTime(level,now+.012);gain.gain.setValueAtTime(level,now+Math.max(.02,release-.12));gain.gain.exponentialRampToValueAtTime(.001,now+release);source.connect(gain).connect(noteBus);trackAudioVoice(source,gain,now);source.start(now);source.stop(now+release+.03)}
function playMidi(midi,instrument,duration,volume=1,startTime=state.audio.currentTime){const epoch=audioEpoch,info=sampleInfo(midi),key=`${instrument}:${info.baseMidi}`,cached=sampleBuffers.get(key);if(cached){playSample(cached,info.rate,instrument,duration,volume,startTime);return}loadSample(instrument,midi).then(({buffer,info:loadedInfo})=>{if(epoch===audioEpoch)playSample(buffer,loadedInfo.rate,instrument,duration,volume,startTime)}).catch(()=>{if(epoch===audioEpoch)playFallbackMidi(midi,duration,volume,startTime)})}
...
async function start(){if(state.running)return;const token=++positionStartToken;await ensureAudio();if(pitchSound.checked){$('#startButton').innerHTML='<span>…</span> 載入音源';await warmPositionSamples()}if(token!==positionStartToken||$('#positionPage').hidden)return;state.running=true;state.started=true;$('#startButton').innerHTML='<span>■</span> 暫停';tick();state.timer=setInterval(tick,60000/+bpm.value/2)}
function pause(){positionStartToken++;clearInterval(state.timer);state.timer=null;state.running=false;stopActiveSounds();$('#startButton').innerHTML='<span>▶</span> 繼續';drawBeatDots()}
...
async function startFretboard(){if(fbState.running)return;const token=++fretboardStartToken;await ensureAudio();if(fbPitchSound.checked){$('#fbStartButton').innerHTML='<span>…</span> 載入音源';await warmFretboardSamples()}if(token!==fretboardStartToken||$('#fretboardPage').hidden)return;fbState.running=true;fbState.started=true;$('#fbStartButton').innerHTML='<span>■</span> 暫停';fretboardTick();fbState.timer=setInterval(fretboardTick,60000/+fbBpm.value)}
function pauseFretboard(){fretboardStartToken++;clearInterval(fbState.timer);fbState.timer=null;fbState.running=false;stopActiveSounds();$('#fbStartButton').innerHTML='<span>▶</span> 繼續';drawFretboardDots(fbState.lastBeat)}
...
function loadSettings(){try{const settings=JSON.parse(localStorage.getItem(SETTINGS_KEY)||'null');if(!settings)return;for(const id of ['metroVolume','noteVolume'])if(settings[id]!=null&&Number.isFinite(+settings[id]))$('#'+id).value=Math.max(0,Math.min(100,+settings[id]));const migratedKeys=settings.keys||[...(settings.majorKeys||[]),...(settings.minorKeys||[])].map(value=>String(value).split('|').at(-1)).filter((value,index,array)=>array.indexOf(value)===index);restoreChecks('#keyChoices',migratedKeys);restoreChecks('#scaleChoices',settings.scales);restoreChecks('#positionChoices',settings.positions);restoreSelect(notationMode,settings.notationMode);restoreSelect(generatorMode,settings.generatorMode);restoreSelect(toneInstrument,settings.toneInstrument);restoreSelect(fbToneInstrument,settings.fbToneInstrument);beatsSelect.value='4';restoreSelect(rhythmSelect,settings.rhythm);if(settings.bpm!=null)bpm.value=settings.bpm;if(settings.fbBpm!=null)fbBpm.value=settings.fbBpm;if(typeof settings.metronomeSound==='boolean')metronomeSound.checked=settings.metronomeSound;if(typeof settings.pitchSound==='boolean')pitchSound.checked=settings.pitchSound;if(typeof settings.fbMetronomeSound==='boolean')fbMetronomeSound.checked=settings.fbMetronomeSound;if(typeof settings.fbPitchSound==='boolean')fbPitchSound.checked=settings.fbPitchSound;if(settings.activeTab==='fretboard')savedActiveTab='fretboard';$('#keyDetails').open=!!settings.keyOpen;$('#scaleDetails').open=!!settings.scaleOpen}catch(error){console.warn('Settings could not be loaded',error)}}
...
for(const id of ['metroVolume','noteVolume'])$('#'+id).oninput=()=>{updateSharedVolumes();saveSettings()};
loadSettings();updateSharedVolumes();setActiveTab(savedActiveTab,false);ensureChecked('#keyChoices','9');ensureChecked('#scaleChoices','major');ensureChecked('#positionChoices','0');$('#bpmValue').textContent=bpm.value;$('#fbBpmValue').textContent=fbBpm.value;restart();resetFretboard(false);saveSettings();
